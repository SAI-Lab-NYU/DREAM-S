...
version_map = {
    "default": "eagle.model.ea_model_head2",
}
...
if args.dataset == "MMT-Bench":
    ...
    ds = load_dataset(
        "parquet", 
        data_files={'test': 'datasets/test/mathvista/data/testmini-00000-of-00001-725687bf7a18d64b.parquet'},
        split="test"
    )
    ds = ds.shuffle(seed=42)
    ds = ds.select(range(sample_num))
...
os.makedirs(f"data/mmt/llava-vicuna-13b/temp1/", exist_ok=True)
answer_file = f"data/mmt/llava-vicuna-13b/temp1/{args.dataset}-{args.version}-top{args.topk}-d{args.depth}-total{args.total_token}-temp{args.temperature}.jsonl"
speed_up_avg = []
ar = []
sd = []
adl = []
...
for i in range(sample_num):
    if args.dataset == "MMT-Bench":
        ...
        image = load_image_path("datasets/test/mathvista",ds[i]["image"])
    elif args.dataset == "DocVQA":
        messages = [
            {"role": "user", "content": [
                {"type": "image"},
